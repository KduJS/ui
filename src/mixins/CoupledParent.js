...
export default function (name) {
  ...
  return {
    ...
    methods: {
       ...
       * @param {object} proxyVm Kdu instance
       */
      $_addCoupledChild (proxyVm) {
        ...
        if (this.$slots && this.$slots.default) {
          ...
          this.$nextTick(() => {
            ...
            const childComponents = this.$slots.default.reduce((list, knode) => {
              const coupledChild = findCoupledChild(knode)
              if (coupledChild) {
       ...
       * @param {object} proxyVm Related child Kdu instance
       */
...
function findCoupledChild (knode) {
  const vm = knode.child
  if (vm) {
